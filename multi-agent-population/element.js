class Element {
  ...
  calculateFitness() {
    ...
    // }
    this.fitness = Math.pow( fitness, 3); //I decided to square fitness because I want to emphesize the small imprevements
  }
  ...
  cross(elementB_){
    let elementA = this;
    let elementB = elementB_;
    let randomIdA = Math.floor(Math.random() * (elementA.data.length/9));
    let randomIdB = Math.floor(Math.random() * (elementB.data.length/9));
    let newElementData = elementA.data.slice(0, randomIdA*9).concat(elementB.data.slice(randomIdB*9, elementB.data.length));
    let newElement = new Element(this.imageWidth, this.imageHeight, newElementData);
